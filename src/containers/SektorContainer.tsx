...
import DeleteSektorPopup from '@/components/Popup/Sektor/DeleteSektorPopup'
...
const SektorContainer = (): JSX.Element => {
  ...
  const [selectedSektor, setSelectedSektor] = useState<number | null>(null)

  const { data: dataSektor, isLoading } = useSWR<ISectorResponse[]>(
    '/master/sektor',
    fetcher
  )

  const filteringDataSektor =
    (dataSektor?.filter(
  ...
  return (
    <Box>
      <DataTable
        ...
        value={
          searchForm
            ? filteringDataSektor?.filter((sektor) => {
                return (
                  sektor?.sektor
                    ?.toLowerCase()
          ...
          body={(rowData: ISectorResponse) => {
            return (
              <Box className="table-content">
                {rowData.sektorsub?.length > 0
                  ? rowData?.sektorsub.map((sektor, index, array) => {
                      const isLast = index === array.length - 1

                      return (
                        <Chip
                          key={sektor.id}
                          label={sektor.sektor_sub || '-'}
                          sx={{
                            mr: !isLast ? '4px' : 0,
            ...
            return (
              <Box display="flex" alignItems="center" px="10px" gap="10px">
                <Button
                  ...
                  onClick={() => {
                    router.push(
                      `/sektor/edit/${encryptText(String(rowData.id))}`
                    )
                  }}
                  variant="contained"
                  sx={{ fontWeight: 600 }}
